...
import {
  fetchAdvertisingSnapshot,
  getAdvertisingMockSnapshot
} from "@/features/advertising/services/advertising-data";
import { FALLBACK_DATA_MESSAGE, formatApiErrorMessage } from "@/shared/api";
import { getDemoAdvertisingSnapshot } from "@/shared/demo/demo-data";
...
export type UseAdvertisingDataResult = {
...
const INITIAL_DATA = getAdvertisingMockSnapshot();

export function useAdvertisingData(): UseAdvertisingDataResult {
  const { enabled: demoModeEnabled } = useDemoMode();
  const [data, setData] = useState<AdvertisingSnapshot>(INITIAL_DATA);
  ...
  useEffect(() => {
    if (demoModeEnabled) {
      const demoSnapshot = getDemoAdvertisingSnapshot();
      setLoading(false);
      setError(null);
    ...
    const timer = window.setTimeout(() => {
      ...
      const run = async () => {
        ...
        try {
          const snapshot = await fetchAdvertisingSnapshot(controller.signal);
          ...
          const fallbackSnapshot = getAdvertisingMockSnapshot();
