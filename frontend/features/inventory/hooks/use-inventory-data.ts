...
} from "@/features/inventory/services/inventory-data";
import { FALLBACK_DATA_MESSAGE, formatApiErrorMessage } from "@/shared/api";
import { getDemoInventorySnapshot } from "@/shared/demo/demo-data";
...
export type UseInventoryDataResult = {
...
const INITIAL_DATA = getInventoryMockSnapshot();

export function useInventoryData(): UseInventoryDataResult {
  const { enabled: demoModeEnabled } = useDemoMode();
  const [data, setData] = useState<InventorySnapshot>(INITIAL_DATA);
  ...
  useEffect(() => {
    if (demoModeEnabled) {
      const demoSnapshot = getDemoInventorySnapshot();
      setLoading(false);
      setError(null);
    ...
    const timer = window.setTimeout(() => {
      ...
      const run = async () => {
        ...
        try {
          const snapshot = await fetchInventorySnapshot(controller.signal);
          ...
          const fallbackSnapshot = getInventoryMockSnapshot();
