...
} from "@/features/products/services/products-data";
import { FALLBACK_DATA_MESSAGE, formatApiErrorMessage } from "@/shared/api";
import { getDemoProductsSnapshot } from "@/shared/demo/demo-data";
...
const INITIAL_DATA = getProductsMockSnapshot();

export function useProductsData(): UseProductsDataResult {
  const { enabled: demoModeEnabled } = useDemoMode();
  const [data, setData] = useState<ProductSnapshot>(INITIAL_DATA);
  ...
  useEffect(() => {
    if (demoModeEnabled) {
      const demoSnapshot = getDemoProductsSnapshot();
      setLoading(false);
      setError(null);
    ...
    const timer = window.setTimeout(() => {
      ...
      const run = async () => {
        ...
        try {
          const snapshot = await fetchProductsSnapshot(controller.signal);
          ...
          const fallbackSnapshot = getProductsMockSnapshot();
