...
def test_wb_native_weekly_mapping_falls_back_to_finance_raw_for_sale_amount_when_payment_report_revenue_is_zero(tmp_path, monkeypatch):
    db_path = _prepare_db(tmp_path)
    conn = sqlite3.connect(db_path)
    try:
        conn.execute(
            "INSERT INTO finance_raw_audit(telegram_id, rrd_id, report_date, deduction, acquiring_fee, raw_json, created_at) "
            "VALUES(42, 'rrd-1', '2026-06-30', 2148.00, 558.14, ?, '2026-07-08 10:00:00')",
            ('{"delivery_rub": 3613.80, "storage_fee": 540.29, "retail_amount": 14046.08, "ppvz_for_pay": 15327.09}',),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    assert snapshot.wb_deductions == 2148.00
    assert snapshot.source_map["wb_sale_amount"]["selected_source"] == "finance_raw_audit.raw_json"
    assert snapshot.source_map["wb_logistics"]["selected_source"] == "payment_reports.delivery"
    assert snapshot.source_map["wb_storage"]["selected_source"] == "payment_reports.storage"
    assert snapshot.source_map["wb_total_to_pay"]["selected_source"] == "payment_reports.bank_payment"
