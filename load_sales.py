    ...
    try:
        ...
        if status != 'SUCCESS':
            _set_last_ads_run_details(telegram_id, {
                ...
                'promotion_count_campaign_ids': [int(advert_id) for advert_id in fetched_ids[:200]],
                'fullstats_requested_ids': [],
                'fullstats_returned_ids': [],
                'missing_advert_ids': [],
            })
        ...
        if batch_status != 'SUCCESS':
            ...
            _set_last_ads_run_details(telegram_id, {
                ...
                'spend_loaded': 0.0,
                'promotion_count_campaign_ids': [int(advert_id) for advert_id in fetched_ids[:200]],
                'fullstats_requested_ids': [int(advert_id) for advert_id in batch_ids[:200]],
                'fullstats_returned_ids': [],
                'missing_advert_ids': [int(advert_id) for advert_id in batch_ids[:200]],
        ...
        details = {
            ...
            'promotion_count_campaign_ids': [int(advert_id) for advert_id in fetched_ids[:200]],
            'fullstats_requested_ids': [int(advert_id) for advert_id in batch_ids[:200]],
            'fullstats_returned_ids': [int(advert_id) for advert_id in sorted(response_advert_ids)[:200]],
