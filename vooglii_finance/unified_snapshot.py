...
def build_unified_financial_snapshot(user_id: int, days, *, context=None, bot=None) -> UnifiedFinancialSnapshot:
    ...
    sales_revenue = _round_money(report_mgmt.get("revenue"))
    ...
    storage = _round_money(report_mgmt.get("storage"))
    acquiring = _round_money(report_mgmt.get("acquiring"))
    wb_deductions = _round_money(report_mgmt.get("deductions"))
    penalties = None
    other_expenses = _round_money(report_mgmt.get("other"))
    unknown_wb_expenses = _round_money(report_mgmt.get("unexplained"))
    ...
    tax_amount = _round_money(after_tax.get("tax"))
    if not bool(after_tax.get("tax_configured")) and tax_amount == 0.0:
        tax_amount = None
    if finance_status != FINANCE_OK and tax_amount == 0.0:
        tax_amount = None

    if tax_amount is not None and profit_before_tax is not None:
        net_profit = round(profit_before_tax - tax_amount, 2)
    else:
        net_profit = _round_money(financial_engine_snapshot.get("official_net_profit"))
    ...
    source_notes = [
        "sales_revenue -> report_mgmt_snapshot / sales loader",
        "orders_amount -> orders snapshot",
        "wb_payout -> report_mgmt_snapshot payout",
        "wb_payments_received -> payment_reconciliation_snapshot",
        "advertising_spend -> advertising_customer_snapshot total_spend",
        "cost_price -> report_mgmt_snapshot cost_price",
        "logistics/storage/acquiring/wb_deductions/other_expenses -> report_mgmt_snapshot",
        "unknown_wb_expenses -> finance difference snapshot",
